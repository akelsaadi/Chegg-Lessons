...
class Divsale {// Get the sales from 6 divisions of 4 quarterlies.
public:
...
int main()
{
	...
	// get quarterly sales.
	d.getQuarterlySales(sales, DIVISIONS);

	// show sales figures for each division.
	d.showTotalPerDivision(sales, DIVISIONS);

	// show increase or decrease for each division.
	d.showDivisionSalesDifferences(sales, DIVISIONS);

	// show totals for each quarter.
	d.ShowTotalPerQuarter(sales, DIVISIONS);

	// show increase or decrease for each quarter.
	d.showQuarterlySalesDifferences(sales, DIVISIONS);

	// average sales.
	d.averageSalesPerQuarter(sales, DIVISIONS);

	// show division with highest sales.
	d.highestSalesDivQuarter(sales, DIVISIONS);
